...
这是一个可复现的中文智能会议助手演示。它使用固定的 LangGraph 工作流创建、查询、修改和安全删除单次会议，也可以直接查询参会人的忙闲状态并按会议主题推荐会议室。
...
创建、修改和删除都必须经过“预览—明确确认—执行”。删除只允许组织者操作一个明确会议；删除所有人会议、删除他人会议、批量删除、清空、批量修改、任意 SQL、任意 HTTP 和提示词注入请求会在写入前被拒绝。LLM 只能输出单会议删除意图，目标唯一性、组织者权限、会议版本和确认哈希由服务端再次校验。
...
首次初始化数据库时会写入一条 Alice 可见的“设计评审”种子会议，并记录种子版本。该会议被删除后不会因服务重启而重新出现。停止服务后删除当前配置的数据库文件，再重新启动即可完整重置：
...
# 查询后安全删除单个会议；预览和成功响应都会显示完整会议详情
Send-Chat 'demo-delete' '查询我的会议'
Send-Chat 'demo-delete' '删除第二个会议'
Send-Chat 'demo-delete' '确认'
Send-Chat 'demo-delete' '查询我的会议'
